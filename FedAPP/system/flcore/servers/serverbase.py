...
class Server(object):
    def __init__(self, args, times):
        ...
        self.goal = args.goal
        self.patience_max = args.patience
        self.top_cnt = 100
        ...
        self.client_queue = queue.PriorityQueue()
        self.patience = 0
        self.min_round = args.min_round
        self.max_acc = -1

    def check_stop(self):
        flag= True
        if len(self.rs_test_acc) <= self.min_round:
            flag = True
        else:
            last_acc = self.rs_test_acc[-1]
    ...
    def select_clients_deay(self):
        selected_clients = []
        for i in range(self.args.num_clients_part):
            time, client = self.client_queue.get()
    ...
    def print_result(self):
        print('-' * 40 + 'Overall Accuracy' + '-' * 40)
        print(self.rs_test_acc)
        print(self.rs_test_acc_time)

        acc = max(self.rs_test_acc)
        print('Final Test: acc={:.2f}'.format(acc * 100))

        thresholds = np.arange(0.2, 1.05, 0.05).tolist()
        result = find_times_for_thresholds(self.rs_test_acc, self.rs_test_acc_time, thresholds)
        print(result)
    ...
    def evaluate(self, acc=None, loss=None):
        ...
        if acc == None:
            ...
            self.rs_test_acc_time.append(time.time()-self.start_time)
        else:
            acc.append(test_acc)
...
def find_times_for_thresholds(accuracy_list, time_list, thresholds):
    times = {}
    for threshold in thresholds:
        times[threshold] = None  # 默认值为 None
        for accuracy, time in zip(accuracy_list, time_list):
